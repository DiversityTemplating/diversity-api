...
handle_get(Req, _State=#state{}) ->
    ...
    case application:get_env(ds_api, stage) of
        undefined ->
            case ds_api_component:exists(ComponentName) of
                true ->
                    handle_component_request(Req1, ds_api_component:dir(ComponentName));
                false ->
...
handle_stage_request(Req, ComponentName, StageEnv) ->
    ...
    case ds_api_component:exists(ComponentName, Stage) of
        true ->
            handle_component_request(Req1, ds_api_component:dir(ComponentName, Stage));
        false ->
            %% Do a fallback call!!
...
handle_component_request(Req, ComponentPath) ->
    ...
    ResultData = case PathInfo of
        undefined ->
            {json, jiffy:encode(ds_api_component:tags(ComponentPath))};
        [PartialTag | Routes] ->
            %% Try to expand the tag into an existing one
            try expand_tag(PartialTag, ds_api_component:tags(ComponentPath)) of
                Tag ->
                    case Routes of
                        [] ->
                            {json,
                             jiffy:encode(ds_api_component:diversity_json(ComponentPath, Tag), [pretty])};
                        [Settings] when Settings =:= <<"settings">> ->
                            {json, jiffy:encode(ds_api_component:settings(ComponentPath, Tag), [pretty])};
                        [Settings] when Settings =:= <<"settingsForm">> ->
                            {json, jiffy:encode(ds_api_component:settingsform(Tag, Settings), [pretty])};
                        [<<"files">> | Path] ->
                            ...
                            Etag = case ds_api:is_production() of
                                false -> <<>>;
                            ...
                            case BrowserEtag of
                                ...
                                _    ->
                                    ...
                                    case ds_api_component:file(ComponentPath, Tag, File) of
                        ...
                        [<<"css">>] ->
                            ...
                            {css, ds_api_component:css(ComponentPath, Tag, Variables)};
                        [<<"thumbnail">>] ->
                            {BrowserEtag, _} = cowboy_req:header(<<"if-none-match">>, Req1),
                            Etag = case ds_api:is_production() of
                                false -> <<>>;
                            ...
                            case BrowserEtag of
                                ...
                                _    -> ds_api_component:thumbnail(ComponentPath)
                            end;
                        _ ->
                            resource_not_found
                    end
